...
export const CiudadesPage = () => {
  ...
  const list = useMemo(() => {
    ...
    return ciudadesList.filter(c => filterEstado === 'ACTIVO' ? c.activo : !c.activo);
  ...
  const resetForm = () => { setCodigo(''); setNombre(''); setDepartamento(''); setActivo(true); setEditingId(null); };

  const startEdit = (c: Ciudad) => {
    setEditingId(c.id); setCodigo(c.codigo); setNombre(c.nombre);
    setDepartamento(c.departamento); setActivo(c.activo);
  ...
  const handleGuardar = (e: React.FormEvent) => {
    ...
    const payload = { codigo, nombre, departamento, activo };
      ...
      <div style={{ background: 'white', borderRadius: '10px', border: '1px solid #e8edf2', padding: '20px 24px 22px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', marginBottom: '24px' }}>
        ...
        <form onSubmit={handleGuardar}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
            <Field label="Código">
              <input value={codigo} onChange={e => setCodigo(e.target.value)} placeholder="Ej. PST" style={inputStyle} onFocus={focusBorder} onBlur={blurBorder} />
            </Field>
            ...
            <Field label="Departamento">
              <input value={departamento} onChange={e => setDepartamento(e.target.value)} placeholder="Ej. Nariño" style={inputStyle} onFocus={focusBorder} onBlur={blurBorder} />
            </Field>
          </div>
          {editingId && (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {[{ l: 'Código', w: '120px' }, { l: 'Nombre', w: '200px' }, { l: 'Departamento', w: '200px' }, { l: 'Estado', w: '100px' }, { l: 'Acciones', w: '90px' }].map(({ l, w }) => (
              ...
              <tbody>
                {paginated.length === 0 ? (
                  ...
                  <tr key={c.id} style={{ borderBottom: '1px solid #f1f5f9' }} onMouseEnter={e => (e.currentTarget.style.background = '#f8fafc')} onMouseLeave={e => (e.currentTarget.style.background = 'white')}>
                    ...
                    <td style={{ padding: '12px 16px', fontSize: '13px', color: '#475569' }}>{c.departamento || '—'}</td>
