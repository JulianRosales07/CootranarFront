...
export const EmpleadosEncomiendasPage = () => {
  ...
  const empleadosMock = useMemo(() => [
    { id: '1', nombre: 'Pedro Ruiz', documento: '1085111222', telefono: '315 111 2222', email: 'pedro@cootranar.com', agencia: 'Terminal Pasto', estado: 'Activo' },
    { id: '2', nombre: 'Laura Díaz', documento: '1085333444', telefono: '316 333 4444', email: 'laura@cootranar.com', agencia: 'Terminal Ipiales', estado: 'Activo' },
    { id: '3', nombre: 'Jorge Muñoz', documento: '1085555666', telefono: '317 555 6666', email: 'jorge@cootranar.com', agencia: 'Terminal Cali', estado: 'Inactivo' },
  ], []);
    ...
    <Layout>
      <div style={{ background: 'white', borderRadius: '10px', border: '1px solid #e8edf2', padding: '20px 24px 22px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px' }}>
          ...
          <span style={{ fontWeight: 700, fontSize: '15px', color: '#0f172a' }}>Registrar Empleado de Encomiendas</span>
        </div>
        <form onSubmit={e => e.preventDefault()}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
            ...
            <Field label="Agencia Asignada" required>
              <select value={agencia} onChange={e => setAgencia(e.target.value)} style={{ ...inputStyle, appearance: 'none' }} onFocus={focusBorder} onBlur={blurBorder}>
                ...
                <option>Terminal Pasto</option><option>Terminal Ipiales</option><option>Terminal Cali</option>
              </select>
            </Field>
          </div>
          ...
          <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'flex-end' }}>
            <button type="submit" style={{ display: 'flex', alignItems: 'center', gap: '8px', background: BLUE, color: 'white', border: 'none', borderRadius: '7px', padding: '10px 20px', fontSize: '13px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
              ...
              <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>save</span> Guardar Empleado
            </button>
          </div>
        </form>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: '10px', border: '1px solid #e8edf2', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '13px 20px', borderTop: '1px solid #f1f5f9', fontSize: '12.5px', color: '#94a3b8' }}>
          Mostrando <strong style={{ color: '#475569' }}>1</strong> a <strong style={{ color: '#475569' }}>3</strong> de <strong style={{ color: '#475569' }}>3</strong> empleados
        </div>
      </div>
    </Layout>
  );
